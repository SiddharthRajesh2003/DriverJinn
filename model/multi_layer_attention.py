...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("TESTING HYBRID AGGREGATOR (LAYER + PATHWAY)")
    print("="*80)
    
    # Simulate GNN outputs
    num_nodes = 100
    hidden_channels = 128
    num_layers = 3
    
    # Create fake GNN outputs
    gnn_outputs = {
        'positive': {
            'one_hop': [torch.randn(num_nodes, hidden_channels) for _ in range(num_layers)],
            'two_hop': [torch.randn(num_nodes, hidden_channels) for _ in range(num_layers)]
        },
        'negative': {
            'one_hop': [torch.randn(num_nodes, hidden_channels) for _ in range(num_layers)],
            'two_hop': [torch.randn(num_nodes, hidden_channels) for _ in range(num_layers)]
    ...
    for method in ['attention', 'hierarchical', 'concat', 'mean']:
        print(f"\n{method.upper()} Aggregation:")
        print("-" * 60)
        
        aggregator = HybridAggregator(
            hidden_channels=hidden_channels,
            num_curvature_types=3,
            num_hop_types=2,
            num_heads=4,
            pathway_aggregation=method,
            dropout=0.1
        )
        
        final_output, attention_info = aggregator(
            gnn_outputs,
            return_attention=True
        )
        
        print(f"  Output shape: {final_output.shape}")
        print(f"  Expected: [{num_nodes}, {hidden_channels}]")
        
        if attention_info:
            if 'layer_attention' in attention_info:
                print(f"  Layer attention computed for {len(attention_info['layer_attention'])} pathways")
            if 'pathway_attention' in attention_info and attention_info['pathway_attention']:
                print(f"  Pathway attention weights:")
                if isinstance(attention_info['pathway_attention'], dict):
                    for pathway, weight in attention_info['pathway_attention'].items():
                        if isinstance(weight, (int, float)):
                            print(f"    {pathway}: {weight:.4f}")
                        else:
                            print(f"    {pathway}: {weight}")
    
    print("\n" + "="*80)
    print("✓ All aggregation methods working!")
    print("\nRECOMMENDATION: Use 'attention' or 'hierarchical' for best performance")
    print("="*80)